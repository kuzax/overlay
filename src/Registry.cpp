...
bool CRegistry::DeleteRegister(LPCSTR lpName)
{
	...
	if (!strlen(lpName))
		return false;
	return !RegDeleteKeyExA(_hKey, lpName, KEY_WOW64_32KEY, NULL);
}
...
void CEasyRegistry::WriteString(LPCSTR lpName, LPCSTR lpString, bool multi_sz, ...)
{
	va_list ap;
...
void CEasyRegistry::WriteBinary(LPCSTR lpName, BYTE* value, int size)
{
	if (value == nullptr) return;
	no_error = SetRegister(lpName, REG_BINARY, value, size);
}
