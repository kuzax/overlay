...
bool MemoryXD::isBadPtr_handlerAny(void* pointer, ULONG size, DWORD dwFlags)
{
	DWORD						dwSize;
...
bool MemoryXD::isBadPtr_readAny(void* pointer, ULONG size)
{
...
int MemoryXD::__page_write(void* _dest, const void* _src, uint32_t len)
{
...
int MemoryXD::__page_read(void* _dest, const void* _src, uint32_t len)
{
	static int	page_size = __page_size_get();
	uint8_t* dest = (uint8_t*)_dest;
...
int MemoryXD::memcpy_safe(void* _dest, const void* _src, uint32_t len, int check, const void* checkdata)
{
	static int		page_size = __page_size_get();
...
int MemoryXD::memset_safe(void* _dest, int c, uint32_t len)
{
	uint8_t* dest = (uint8_t*)_dest;
	uint8_t buf[4096];
...
int MemoryXD::memcmp_safe(const void* _s1, const void* _s2, uint32_t len)
{
...
void* MemoryXD::memdup(const void* src, int len)
{
...
void MemoryXD::HookInstallCall(DWORD dwInstallAddress, DWORD dwHookFunction)
{
...
void MemoryXD::CreateJump(BYTE* pAddress, DWORD dwJumpTo, DWORD dwLen)
{
