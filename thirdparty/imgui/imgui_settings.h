...
namespace utils
{
	inline float CalculateTextWidthWithoutColorCodes(const ImFont* font, const char* text_begin, const char* text_end = nullptr) {
		if (!text_end)
			text_end = text_begin + strlen(text_begin); // Если text_end не указан, вычисляем его
		...
		while (s < text_end) {
			if (*s == '^' && (s + 1) < text_end && *(s + 1) >= '0' && *(s + 1) <= '9') {
				// Пропускаем цветовой код (^ и следующую цифру)
				s += 2;
				continue;
			}

			// Декодируем символ UTF-8
			...
			if (c < 0x80) {
				...
				if (c == 0) // Некорректный UTF-8
					break;
			}

			// Получаем глиф и добавляем его ширину к общей ширине текста
...
inline namespace c
{
	...
	inline ImColor second_color = ImColor(22, 22, 24, 255);
	inline ImColor background_color = ImColor(22, 22, 27, 255);
	inline ImColor stroke_color = ImColor(46, 48, 56, 0);
	inline ImColor window_bg_color = ImColor(14, 14, 16, 200);

	inline ImVec4 separator = ImColor(40, 42, 52, 255);

	inline namespace anim
	{
		inline float   speed = 1.0f;
		inline ImColor active = ImColor(230, 131, 90, 255);
		inline ImColor default = ImColor(27, 27, 32, 200);
	}
	...
	inline namespace child
	{
		...
		inline ImVec4 background = ImColor(19, 19, 23, 200);
		inline ImVec4 stroke = ImColor(47, 48, 55, 60);
		inline float   rounding = 4.f;
	}

	namespace page
	{
		inline ImVec4 background_active = ImColor(37, 39, 53, 255);
		inline ImVec4 background = ImColor(31, 33, 40, 255);
		inline ImVec4 text_hov = ImColor(240, 240, 240, 255);
		inline ImVec4 text = ImColor(224, 224, 224, 255);
	...
	inline namespace elements
	{
		...
		inline ImVec4 background_hovered = ImColor(44, 46, 52, 255);
		inline ImVec4 background = ImColor(39, 41, 47, 255);
		inline float   rounding = 3.f;
	}

	inline namespace checkbox
	{
		inline ImVec4 mark = ImColor(59, 130, 246, 255);
	}
	...
	inline namespace text
	{
		inline namespace label
		{
			...
			inline ImColor hovered = ImColor(235, 235, 240, 255);
			inline ImColor default = ImColor(185, 185, 185, 255);
		}
		inline namespace description
		{
			inline ImColor active = ImColor(180, 180, 185, 255);
			inline ImColor hovered = ImColor(160, 160, 170, 255);
			inline ImColor default = ImColor(140, 140, 150, 255);
		}
	}
}
...
inline void UpdateTheme(bool bTheme, float t)
{
	...
	// Текстовые и акцентные цвета оставляем непрозрачными
	...
	// Цвет чекбокса и текста тоже можно не трогать
	...
	c::text::label::hovered = ImLerpColor(
		...
		t);
	c::text::label::default = ImLerpColor(
		c::text::label::default,
	...
	c::text::description::hovered = ImLerpColor(
		...
		t);
	c::text::description::default = ImLerpColor(
